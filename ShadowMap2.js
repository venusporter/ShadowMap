 ...
 * Name: Venus Porter
 * Professor: shaw
 * Assignment #: mod 7 assignment 2 lab 1 
 */
"use strict";
    ...
    document.getElementById("selObject").onchange =
        function (event) {
            ...
            if (objSelected == "0") {
                ...
                let startV = pointsArray.length;
                loadobj(objData);
                for (let nextV = startV; nextV < pointsArray.length; ++nextV)
...
function createPlane() {
    ...
    let t1 = subtract(planeVertices[1], planeVertices[0]);
    let t2 = subtract(planeVertices[1], planeVertices[2]);
...
function quad(a, b, c, d) {
    let t1 = subtract(cubeVertices[a], cubeVertices[b]);
    let t2 = subtract(cubeVertices[a], cubeVertices[c]);
    let normal = vec3(cross(t1, t2));
